...
const API_BASE = "https://barbavape-api.onrender.com";
...
function LoyaltyCard({ client }) {
  ...
  return (
    <div style={{ background:`linear-gradient(135deg,${GD} 0%,${G} 60%,${GL} 100%)`, borderRadius:20, padding:"24px 24px 20px", color:CREAM, position:"relative", overflow:"hidden", boxShadow:"0 8px 32px rgba(15,33,24,0.25)" }}>
      ...
      {client.welcomeGiven && (
        <div style={{ display:"inline-flex", alignItems:"center", gap:6, background:"rgba(200,168,75,0.25)", border:"1px solid rgba(200,168,75,0.5)", borderRadius:10, padding:"4px 10px", fontSize:11, marginBottom:8 }}>
          <span>🎁</span><span style={{ fontWeight:600 }}>+50 pts de bienvenue offerts</span>
        </div>
      )}
      {next ? (<>
        <div style={{ display:"flex", justifyContent:"space-between", fontSize:11, opacity:0.65, marginBottom:6 }}><span>Vers {next.name}</span><span>{next.min-(client.points||0)} pts restants</span></div>
...
function CigaretteTab({ client, setClient }) {
  ...
  const pctReduction = client.cigsBefore>0 ? Math.round(((client.cigsBefore-(client.cigsNow||0))/client.cigsBefore)*100) : 0;
  ...
  const circleSize=150, r=54, cx=circleSize/2, cy=circleSize/2;
    ...
    <div>
        ...
        <div style={{ display:"flex", gap:10, marginBottom:14 }}>
          <div style={{ background:"white", borderRadius:16, border:"1px solid #eee", flex:1, padding:"16px 12px", display:"flex", flexDirection:"column", alignItems:"center" }}>
            <svg width={circleSize} height={circleSize}>
              ...
              <circle cx={cx} cy={cy} r={r} fill="none" stroke={pctReduction===100?"#34C759":G} strokeWidth={12} strokeDasharray={circ} strokeDashoffset={dashOffset} strokeLinecap="round" transform={`rotate(-90 ${cx} ${cy})`}/>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:8, flex:1 }}>
            {[["🗓️",`${daysElapsed}`,"jours","de vape"],["🚬",cigsAvoided.toLocaleString(),"cig.","évitées"],["💶",`${moneySaved}€`,"","économisés"]].map(([icon,val,unit,label])=>(
      ...
      {client.quitDate && (
        <div style={{ marginBottom:14 }}>
          {lastMilestone && <div style={{ background:"#edf7f1", border:"1.5px solid #9dd4b4", borderRadius:14, padding:"13px 16px", marginBottom:8, display:"flex", alignItems:"center", gap:12 }}><span style={{ fontSize:28 }}>{lastMilestone.icon}</span><div><div style={{ fontWeight:700, fontSize:14, color:G }}>✓ {lastMilestone.label}</div><div style={{ fontSize:12, color:"#555" }}>{lastMilestone.desc}</div></div></div>}
          {nextMilestone && <div style={{ background:"#f9f9f9", border:"1.5px solid #eee", borderRadius:14, padding:"13px 16px", display:"flex", alignItems:"center", gap:12 }}><span style={{ fontSize:28, opacity:0.45 }}>{nextMilestone.icon}</span><div style={{ flex:1 }}><div style={{ fontWeight:600, fontSize:13, color:"#777" }}>Prochain cap : {nextMilestone.label}</div><div style={{ fontSize:12, color:"#bbb" }}>{nextMilestone.desc}</div><div style={{ background:"#eee", borderRadius:6, height:5, marginTop:8, overflow:"hidden" }}><div style={{ background:G, height:"100%", width:`${Math.min((hoursElapsed/nextMilestone.hours)*100,100)}%`, borderRadius:6 }}/></div></div></div>}
        </div>
      )}
      {!editing ? (
        ...
        <div style={{ background:"white", border:"1.5px solid #ddd", borderRadius:16, padding:18 }}>
          ...
          {[{label:"📅 Date de passage à la vape",field:"quitDate",type:"date"},{label:"🚬 Cigarettes/jour avant la vape",field:"cigsBefore",type:"number",min:1,max:60},{label:"🚬 Cigarettes/jour aujourd'hui",field:"cigsNow",type:"number",min:0,max:60},{label:"💶 Prix du paquet (€)",field:"prixPaquet",type:"number",min:5,max:20}].map(f=>(
            <div key={f.field} style={{ marginBottom:14 }}>
              <div style={{ fontSize:13, fontWeight:600, color:"#555", marginBottom:6 }}>{f.label}</div>
              <input type={f.type} value={form[f.field]} min={f.min} max={f.max} onChange={e=>setForm({...form,[f.field]:f.type==="number"?parseInt(e.target.value)||0:e.target.value})} style={{ width:"100%", padding:"10px 14px", borderRadius:10, border:"1.5px solid #ddd", fontSize:15, boxSizing:"border-box", outline:"none" }}/>
...
function RewardsTab({ client }) {
  return <div>{REWARDS.map(r=>{const ok=client.points>=r.points;return(<div key={r.id} style={{ display:"flex", alignItems:"center", padding:"14px 16px", marginBottom:8, background:ok?"#edf7f1":"#fafafa", borderRadius:14, border:`1.5px solid ${ok?"#9dd4b4":"#e8e8e8"}` }}><span style={{ fontSize:26, marginRight:14 }}>{r.icon}</span><div style={{ flex:1 }}><div style={{ fontWeight:600, fontSize:14 }}>{r.name}</div><div style={{ fontSize:12, color:"#999" }}>{r.points} points</div></div>{ok?<span style={{ background:G, color:"white", padding:"5px 12px", borderRadius:10, fontSize:12, fontWeight:700 }}>Dispo ✓</span>:<span style={{ color:"#bbb", fontSize:13, fontWeight:600 }}>−{r.points-client.points} pts</span>}</div>);})}</div>;
}
...
function ContactEquipeTab({ client }) {
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", gap:8, marginBottom:16 }}>{Object.values(STORES).map(st=>(<div key={st.key} onClick={()=>setSel(st.key)} style={{ flex:1, padding:"11px 8px", borderRadius:12, border:`2px solid ${sel===st.key?st.color:"#eee"}`, background:sel===st.key?`${st.color}18`:"white", cursor:"pointer", textAlign:"center" }}><div style={{ fontSize:22 }}>{st.icon}</div><div style={{ fontWeight:700, fontSize:12, marginTop:3, color:sel===st.key?st.color:"#555" }}>{st.name}</div><div style={{ fontSize:10, color:"#bbb", marginTop:1 }}>{st.address}</div></div>))}</div>
      <div style={{ background:`${s.color}12`, border:`1.5px solid ${s.color}44`, borderRadius:14, padding:"12px 16px", marginBottom:16 }}><div style={{ fontWeight:700, color:s.color, fontSize:14 }}>{s.icon} {s.name} — {s.address}</div></div>
      <div style={{ fontSize:13, color:"#555", fontWeight:600, marginBottom:8 }}>📞 Téléphone</div>
      <div onClick={()=>open(`tel:${s.phone}`)} style={{ display:"flex", alignItems:"center", padding:"14px 16px", marginBottom:16, background:"#FFF4E6", border:"1.5px solid #FF9500", borderRadius:14, cursor:"pointer" }}><span style={{ fontSize:24, marginRight:14 }}>📞</span><div style={{ flex:1 }}><div style={{ fontWeight:700, fontSize:14, color:"#FF9500" }}>Appeler la boutique</div><div style={{ fontSize:12, color:"#aaa", marginTop:2 }}>{s.phone}</div></div><span style={{ fontSize:22, color:"#FF9500" }}>›</span></div>
  ...
  return (
    <div style={{ maxWidth:430, margin:"0 auto" }}>
      ...
      <div style={{ display:"flex", gap:2, margin:"12px 0", background:"#e8e8ec", borderRadius:12, padding:3 }}>
        {tabs.map(([k,icon])=>(<button key={k} onClick={()=>setTab(k)} style={{ flex:1, padding:"8px 2px", borderRadius:9, border:"none", cursor:"pointer", fontSize:tab===k?11:17, fontWeight:tab===k?700:400, background:tab===k?G:"transparent", color:tab===k?CREAM:"#888", transition:"all 0.2s", lineHeight:1.2 }} title={labels[k]}>{tab===k?labels[k]:icon}</button>))}
      </div>
...
function AdminView({ clients, setClients }) {
  ...
  const getFiltered = ()=>clients.filter(c=>{
    ...
    if(sortBy==="level") return (b.points||0)-(a.points||0);
  ...
  function doAddPoints() {
    ...
    setClients(prev=>prev.map(c=>c.id===sel.id?{...c,points:(c.points||0)+n}:c));
    setSel(prev=>({...prev,points:(prev.points||0)+n}));
    setAddPts(""); showToast(`+${n} points ajoutés à ${sel.name} ✓`);
  }

  function giveWelcomeBonus(client) {
    setClients(prev=>prev.map(c=>c.id===client.id?{...c,points:(c.points||0)+WELCOME_BONUS,welcomeGiven:true}:c));
    setSel(prev=>({...prev,points:(prev.points||0)+WELCOME_BONUS,welcomeGiven:true}));
    showToast(`🎁 ${WELCOME_BONUS} pts de bienvenue offerts à ${client.name} !`);
  }
    ...
    <div style={{ maxWidth:480, margin:"0 auto", position:"relative" }}>
      ...
      <div style={{ background:`linear-gradient(135deg,${GD},${G})`, color:CREAM, padding:"18px 20px", borderRadius:18, marginBottom:14 }}>
        ...
        <div style={{ display:"flex", gap:8, marginTop:10 }}>
          {Object.values(STORES).map(s=>(<div key={s.key} style={{ background:`${s.color}33`, border:`1px solid ${s.color}66`, borderRadius:8, padding:"4px 10px", fontSize:11, fontWeight:600 }}>{s.icon} {s.short} · {clients.filter(c=>c.store===s.key).length} clients</div>))}
        </div>
      </div>
      <div style={{ display:"flex", gap:3, background:"#e8e8ec", borderRadius:12, padding:3, marginBottom:14 }}>
        {[["dashboard","📊 Tableau"],["clients","👥 Clients"],["messages","💌 Messages"]].map(([k,l])=>(<button key={k} onClick={()=>{setTab(k);setSel(null);}} style={{ flex:1, padding:"9px 4px", borderRadius:9, border:"none", cursor:"pointer", fontSize:12, fontWeight:tab===k?700:500, background:tab===k?G:"transparent", color:tab===k?CREAM:"#666", transition:"all 0.2s" }}>{l}</button>))}
      </div>

      {tab==="dashboard"&&(
        <div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:16 }}>
            {[["👥","Clients",clients.length],["⭐","Points distribués",clients.reduce((s,c)=>s+(c.points||0),0).toLocaleString()],["🛒","Achats",clients.reduce((s,c)=>s+(c.purchases||[]).length,0)],["🏆","Gold ou +",clients.filter(c=>(c.points||0)>=500).length]].map(([icon,label,val])=>(<div key={label} style={{ background:"white", border:"1px solid #eee", borderRadius:14, padding:"16px 14px", textAlign:"center" }}><div style={{ fontSize:26, marginBottom:4 }}>{icon}</div><div style={{ fontSize:24, fontWeight:900, color:G }}>{val}</div><div style={{ fontSize:11, color:"#999", marginTop:2 }}>{label}</div></div>))}
          </div>
          {clients.slice(0,20).map(c=>{const lvl=getLevel(c.points||0),d=daysSince(c);const pctRed=c.cigsBefore>0?Math.round(((c.cigsBefore-(c.cigsNow||0))/c.cigsBefore)*100):null;return(<div key={c.id} onClick={()=>{setSel(c);setTab("clients");}} style={{ display:"flex", alignItems:"center", padding:"13px 16px", marginBottom:8, background:"white", borderRadius:14, border:"1px solid #eee", cursor:"pointer" }}><Avatar name={c.name}/><div style={{ flex:1, marginLeft:12 }}><div style={{ fontWeight:600, fontSize:14 }}>{c.name}</div><div style={{ fontSize:12, color:"#aaa", marginTop:2 }}>{c.points||0} pts · <span style={{ color:absentColor(d) }}>⏳ {absentLabel(d)}</span>{pctRed!==null&&<span style={{ color:pctRed===100?"#34C759":G, marginLeft:8 }}>🚬 -{pctRed}%</span>}</div></div><div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:4 }}><span style={{ background:lvl.color, color:"#111", padding:"3px 10px", borderRadius:12, fontSize:11, fontWeight:700 }}>{lvl.name}</span><StoreBadge storeKey={c.store} small/></div></div>);})}
        </div>
      )}
      ...
      {tab==="clients"&&!selData&&(
        <div>
          ...
          <div style={{ display:"flex", gap:6, marginBottom:10 }}>
            {[["all","Tous"],["chateau","🏰 Château"],["ecommoy","🛒 Ecommoy"]].map(([k,l])=>(<button key={k} onClick={()=>setStoreFilter(k)} style={{ padding:"7px 12px", borderRadius:20, border:"none", cursor:"pointer", fontSize:11, fontWeight:storeFilter===k?700:500, background:storeFilter===k?G:"#eee", color:storeFilter===k?CREAM:"#555" }}>{l}</button>))}
          </div>
          <div style={{ marginBottom:14 }}>
            ...
            <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
              {SORT_OPTIONS.map(([k,l])=>(<button key={k} onClick={()=>setSortBy(k)} style={{ padding:"7px 13px", borderRadius:20, border:`1.5px solid ${sortBy===k?G:"#ddd"}`, cursor:"pointer", fontSize:12, fontWeight:sortBy===k?700:400, background:sortBy===k?G:"white", color:sortBy===k?CREAM:"#666" }}>{l}</button>))}
            </div>
          </div>
          {filtered.length===0&&<div style={{ textAlign:"center", color:"#bbb", padding:32 }}>Aucun client trouvé</div>}
          {filtered.map(c=>{const lvl=getLevel(c.points||0),d=daysSince(c),pctRed=c.cigsBefore>0?Math.round(((c.cigsBefore-(c.cigsNow||0))/c.cigsBefore)*100):null;return(<div key={c.id} onClick={()=>setSel(c)} style={{ display:"flex", alignItems:"center", padding:"14px 16px", marginBottom:8, background:"white", borderRadius:14, border:"1px solid #eee", cursor:"pointer" }}><Avatar name={c.name}/><div style={{ flex:1, marginLeft:12 }}><div style={{ display:"flex", alignItems:"center", gap:6, flexWrap:"wrap" }}><span style={{ fontWeight:600, fontSize:14 }}>{c.name}</span><StoreBadge storeKey={c.store} small/></div><div style={{ fontSize:12, color:"#aaa", marginTop:3 }}>{c.points||0} pts · {c.phone}{pctRed!==null&&<span style={{ color:pctRed===100?"#34C759":G, marginLeft:6 }}>🚬 -{pctRed}%</span>}</div></div><div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:5 }}><span style={{ background:lvl.color, color:"#111", padding:"3px 10px", borderRadius:12, fontSize:11, fontWeight:700 }}>{lvl.name}</span><span style={{ color:absentColor(d), fontSize:11, fontWeight:700 }}>⏳ {absentLabel(d)}</span></div></div>);})}
        </div>
      )}

      {tab==="clients"&&selData&&(()=>{
        const c=selData,lvl=getLevel(c.points||0),pref=ADMIN_CHANNELS.find(ch=>ch.key===c.preferredChannel),st=STORES[c.store];
        ...
        return(<div>
          ...
          <div style={{ background:`linear-gradient(135deg,${G},${GL})`, borderRadius:18, padding:20, color:CREAM, marginBottom:12 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
              <div><div style={{ fontSize:20, fontWeight:800 }}>{c.name}</div><div style={{ fontSize:12, opacity:0.65, marginTop:4 }}>{c.phone} · {c.email}</div>{st&&<div style={{ marginTop:8, display:"inline-block", background:`${st.color}33`, border:`1px solid ${st.color}66`, borderRadius:14, padding:"4px 12px", fontSize:12, fontWeight:700 }}>{st.icon} {st.name}</div>}{pctRed!==null&&<div style={{ marginTop:6, fontSize:12, opacity:0.85 }}>🚬 Réduction tabac : <strong>{pctRed}%</strong></div>}</div>
          ...
          <div style={{ background:"white", borderRadius:14, border:"1px solid #eee", padding:16, marginBottom:10 }}>
            ...
            <div style={{ display:"flex", gap:8 }}><input type="number" value={addPts} onChange={e=>setAddPts(e.target.value)} placeholder="Montant achat (€)" style={{ flex:1, padding:"10px 14px", borderRadius:10, border:"1.5px solid #ddd", fontSize:14, outline:"none" }}/><button onClick={doAddPoints} style={{ background:addPts&&parseInt(addPts)>0?G:"#ccc", color:"white", border:"none", borderRadius:10, padding:"10px 16px", cursor:"pointer", fontWeight:700, fontSize:14 }}>+{addPts?Math.max(0,parseInt(addPts)||0):0} pts</button></div>
            <div style={{ fontSize:11, color:"#bbb", marginTop:6 }}>1€ = 1 point</div>
            {!c.welcomeGiven&&<button onClick={()=>giveWelcomeBonus(c)} style={{ width:"100%", marginTop:10, padding:"11px", background:`linear-gradient(135deg,${GOLD},#e0a830)`, color:"#111", border:"none", borderRadius:10, fontWeight:800, cursor:"pointer", fontSize:14 }}>🎁 Offrir les 50 pts de bienvenue</button>}
            {c.welcomeGiven&&<div style={{ marginTop:10, padding:"9px 14px", background:"#f0faf4", borderRadius:10, fontSize:12, color:"#555", display:"flex", alignItems:"center", gap:6 }}><span>✅</span><span>Cadeau de bienvenue déjà offert (+50 pts)</span></div>}
          </div>
          <div style={{ background:"white", borderRadius:14, border:"1px solid #eee", padding:16, marginBottom:10 }}>
          ...
          {template&&(
            <div>
              <div style={{ display:"flex", alignItems:"center", gap:6, margin:"12px 0 10px", flexWrap:"wrap" }}>
                ...
                {[["all","Tous"],["chateau","🏰 Château"],["ecommoy","🛒 Ecommoy"]].map(([k,l])=>(<button key={k} onClick={()=>setMsgStore(k)} style={{ padding:"5px 12px", borderRadius:16, border:"none", cursor:"pointer", fontSize:11, fontWeight:msgStore===k?700:500, background:msgStore===k?G:"#eee", color:msgStore===k?CREAM:"#555" }}>{l}</button>))}
              </div>
              {clients.filter(c=>msgStore==="all"||c.store===msgStore).map(c=>{const pref=ADMIN_CHANNELS.find(ch=>ch.key===c.preferredChannel);const ready=template.id!==5||customMsg.trim().length>2;return(<div key={c.id} style={{ display:"flex", alignItems:"center", padding:"12px 16px", marginBottom:8, background:"white", borderRadius:14, border:"1px solid #eee" }}><Avatar name={c.name} size={38}/><div style={{ flex:1, marginLeft:12 }}><div style={{ display:"flex", alignItems:"center", gap:6 }}><span style={{ fontWeight:600, fontSize:14 }}>{c.name}</span><StoreBadge storeKey={c.store} small/></div><div style={{ fontSize:11, color:"#aaa", marginTop:2 }}>{pref?.icon} via {pref?.label}</div></div><button onClick={()=>ready&&pref&&sendMsg(c,pref)} style={{ background:ready&&pref?(pref.color||G):"#ddd", color:"white", border:"none", borderRadius:10, padding:"9px 16px", cursor:ready&&pref?"pointer":"default", fontSize:13, fontWeight:700 }}>Envoyer</button></div>);})}
            </div>
          )}
        </div>
      )}
...
export default function App() {
  ...
  if (loading) return (
    <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", background:"#f2f2f7", fontFamily:"system-ui" }}>
      <div style={{ textAlign:"center" }}>
        <div style={{ fontSize:48, marginBottom:16 }}>🐊</div>
        <div style={{ fontWeight:700, color:G, fontSize:18 }}>Chargement des clients...</div>
        <div style={{ color:"#aaa", fontSize:13, marginTop:8 }}>Connexion à Hiboutik en cours</div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#f2f2f7", fontFamily:"'Inter',system-ui,-apple-system,sans-serif", padding:"16px 12px 40px" }}>
      <div style={{ display:"flex", gap:3, background:"#e0e0e6", borderRadius:14, padding:4, maxWidth:480, margin:"0 auto 18px" }}>
        {[["client","🐊 Vue Client"],["admin","⚙️ Vue Admin"]].map(([k,l])=>(<button key={k} onClick={()=>setMode(k)} style={{ flex:1, padding:"11px", borderRadius:11, border:"none", cursor:"pointer", fontWeight:mode===k?700:500, background:mode===k?G:"transparent", color:mode===k?CREAM:"#555", fontSize:14, transition:"all 0.2s" }}>{l}</button>))}
      </div>
      {mode==="client"
        ? (activeClient?<ClientView client={activeClient} setClient={setActiveClient}/>:<div style={{ textAlign:"center", padding:40, color:"#aaa" }}>Aucun client</div>)
